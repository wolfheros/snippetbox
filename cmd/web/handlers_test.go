...
func TestShowSnippet(t *testing.T) {
	// Create a application struct instance filled with mock data.
	app := newTestApplication(t)

	// Establish a new test server for running end-to-end test.
	ts := newTestServer(t, app.routes())
	defer ts.Close()

	// Set up some table-driven tests to check response sent by server for different urls
	tests := []struct {
		name     string
		urlPath  string
		wantCode int
...
func TestSignupUser(t *testing.T) {
	// Creating a application struct instance containning our mocked denpendcis
	// and set up test server
	app := newTestApplication(t)
	ts := newTestServer(t, app.routes())
	defer ts.Close()

	// Make a GET /user/sigup request and then extract the CSRF token
	_, _, body := ts.get(t, "/user/signup")
	csrfToken := extractCSRFToken(t, body)

	// setting test anomosy struct for testing.
	tests := []struct {
		name         string
		userName     string
		userEmail    string
